...
graph TD
    ...
    A -->|2. HTTP POST /api/transcribe| C[CoAct Go Backend]
    A -->|3. HTTP POST /api/session/.../chat| C
    C -->|4. HTTP POST /api/speak| D[Piper / OpenAI TTS]
...
| **PDF Viewer** | `react-native-pdf` & `react-native-blob-util` | Downloads and renders the professional Go-generated PDF reports. |
...
Once the session is completed and the Go backend compiles the PDF, display it in a fully native reader using `react-native-pdf`.
...
3. **Local TTS (Offline Voice):**
   * Integrate React Native's native Text-to-Speech library (`react-native-tts`) to leverage the mobile OS native voices (Siri/Google voices), bypassing the need to run Piper or store complex voice binaries.

---

## 5. Development Connection Setup (Bridge to Local Go Backend)
When testing the React Native application on a physical mobile device, the phone must be able to reach your Go backend running on `localhost:5000`.

* **If using Android Emulator:** Use IP `http://10.0.2.2:5000` to represent the host machine.
* **If using Physical Android Device:** Connect your phone via USB, enable Developer Mode, and run:
  ```bash
  adb reverse tcp:5000 tcp:5000
  ```
  Now, your mobile app can make requests directly to `http://localhost:5000`.
* **If using iOS Device:** Ensure your laptop and iPhone are connected to the exact same Wi-Fi network, and use your laptop's local IP address (e.g. `http://192.168.1.XX:5000`).
