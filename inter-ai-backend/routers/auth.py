...
async def transcribe_audio(request: Request):
    ...
    try:
        ...
        try:
            print(f" [INFO] Transcribing audio with Local Whisper Server...")
            import httpx
            ...
            if resp.status_code != 200:
                print(f" [ERROR] Local Whisper STT Error: {resp.status_code} {resp.text}")
...
async def speak_text(request: Request, _ = Depends(standard_limiter)):
    ...
    try:
        ...
        print(f" [INFO] Generating TTS via Local Piper for: '{text[:80]}...'")
        ...
        try:
            ...
            if process.returncode != 0:
                print(f" [WARNING] Piper TTS Error: {process.stderr}")
            ...
            if not audio_data or len(audio_data) < 100:
                print(" [WARNING] Piper TTS Error: No audio generated")
                return JSONResponse(content={"error": "No audio generated"}, status_code=500)
            
            mimetype = "audio/wav"
            print(f" [SUCCESS] Local Piper TTS generated {len(audio_data)} bytes")
