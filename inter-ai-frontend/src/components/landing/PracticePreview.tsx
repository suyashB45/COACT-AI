        ...
        <section className="ds-section py-20 md:py-28 border-b border-white/[0.05]">
            ...
            <div className="relative z-10 container mx-auto max-w-6xl items-center gap-12 px-6 grid lg:grid-cols-[0.9fr_1.1fr] lg:gap-20">
                ...
                >
                    ...
                    <h2 className="text-4xl md:text-5xl font-black text-white leading-[1.08] tracking-tight"
                            ...
                            <button
                                ...
                                className={`rounded-full px-4 py-2 text-sm font-semibold transition-all duration-200 ${
                                    scenarioKey === key
                                        ...
                                        : 'border border-white/[0.08] text-white/40 hover:text-white/70 hover:border-white/20 bg-white/[0.03]'
                                }`}
                            >
                                {scenarios[key].label}
                            </button>
                ...
                <motion.div
                    ...
                    className="overflow-hidden rounded-2xl border border-white/[0.08] shadow-[0_24px_60px_rgba(0,0,0,0.5)]"
                    style={{ background: 'rgba(255,255,255,0.03)' }}
                >
                    {/* Panel header */}
                    <div className="flex items-center justify-between border-b border-white/[0.07] px-5 py-4">
                        <div className="flex items-center gap-3">
                            <div className="flex h-9 w-9 items-center justify-center rounded-full bg-violet-500/20 border border-violet-500/30 text-violet-400">
                                <Mic className="h-4 w-4" />
                            </div>
                            <div>
                                <p className="text-sm font-semibold text-white">Practice session</p>
                                <p className="text-[11px] text-white/40">{scenario.label}</p>
                            </div>
                        </div>
                        ...
                        <AnimatePresence mode="wait">
                            ...
                            >
                                ...
                                <div className="flex items-end gap-3">
                                    ...
                                    <div>
                                        <p className="mb-1.5 text-[11px] font-semibold text-white/35">
                                            {scenario.person} · {scenario.role}
                                        </p>
                                        <p className="max-w-sm rounded-2xl rounded-bl-sm px-4 py-3 text-[14px] leading-relaxed text-white/80"
                                ...
                                {/* Choices */}
                                <div className="pl-11">
                                    <p className="mb-2 text-[11px] font-semibold text-white/35 uppercase tracking-wider">Choose your response</p>
                                            ...
                                            <button
                                                ...
                                                className={`w-full rounded-xl border px-3.5 py-2.5 text-left text-[13px] font-medium transition-all duration-200 ${
                                                    selected === idx
                                                        ...
                                                        : 'border-white/[0.07] text-white/45 hover:border-violet-500/30 hover:bg-violet-500/[0.08] hover:text-white/80 disabled:opacity-50'
                                                }`}
                                            >
                                                {choice}
                                            </button>
                                ...
                                {selected !== null && (
                                    <div className="flex justify-end">
                                        <p className="max-w-sm rounded-2xl rounded-br-sm px-4 py-3 text-[13px] leading-relaxed text-white font-medium"
                                            style={{ background: 'linear-gradient(135deg, rgba(139,92,246,0.35), rgba(96,165,250,0.25))', border: '1px solid rgba(139,92,246,0.3)' }}>
                                ...
                                {isThinking && (
                                    <div className="pl-11">
                                        <span className="inline-flex gap-1 rounded-full px-3 py-2" style={{ background: 'rgba(255,255,255,0.05)' }}>
                                            {[0, 1, 2].map((d) => (
                                                <i key={d} className="h-1.5 w-1.5 rounded-full bg-white/40 animate-bounce"
                                ...
                                {selected !== null && !isThinking && (
                                    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-3">
                                        <div className="flex items-end gap-3">
                                            ...
                                            <p className="max-w-sm rounded-2xl rounded-bl-sm px-4 py-3 text-[13px] leading-relaxed text-white/75"
                                                style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.08)' }}>
                                                {scenario.response}
                                            </p>
                                        </div>
                                        {/* Coaching note */}
                                        <p className="ml-11 flex items-start gap-2 rounded-xl border border-emerald-500/20 bg-emerald-500/[0.07] p-3 text-[12px] leading-relaxed text-emerald-300">
                    ...
                    {selected !== null && (
                        <div className="border-t border-white/[0.06] px-5 py-3">
                            <button
                                ...
                                className="inline-flex items-center gap-2 text-xs font-medium text-white/35 hover:text-white/70 transition-colors"
                            >
